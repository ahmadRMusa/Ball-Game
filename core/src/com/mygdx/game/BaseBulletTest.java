...
public class BaseBulletTest extends BulletTest {
	...
	public static boolean shadows = true; // enable shadows
	...
	public static void init() {
		...
		if (Gdx.app.getType() == ApplicationType.Desktop && customDesktopLib != null) {
			...
			System.out.println(customDesktopLib);
		} else
			Bullet.init();
	...
	public void create() {
		...
		if (width > height) {
			...
			playerCamera = new OrthographicCamera(3f * width / height, 3f);
		} else {
			camera = new PerspectiveCamera(67f, 3f, 3f * height / width);
			playerCamera = new OrthographicCamera(3f, 3f * height / width);
		}

		//		camera.position.set(10f, 10f, 10f); // 10, 10, 10
		//		camera.lookAt(0, 0, 0);
		//		camera.near = 1.1f;
		//		camera.far = 150.0f;
		//		camera.update();
		//		
		//		playerCamera.position.set(10f, 10f, 10f); // 10, 10, 10 not used yet
		//		playerCamera.lookAt(0, 0, 0);
		//		playerCamera.near = 1.1f;
		//		playerCamera.far = 150.0f;
		//		playerCamera.update();
		...
		final Model sphereModel = modelBuilder.createSphere(1f, 1f, 1f, 16, 16, new Material(ColorAttribute.createDiffuse(Color.WHITE),
				ColorAttribute.createSpecular(Color.WHITE)), Usage.Position | Usage.Normal);
		...
		world.addConstructor("sphere", new BulletConstructor(sphereModel, 1f)); // mass = 1kg: dynamic body
		world.addConstructor("staticsphere", new BulletConstructor(sphereModel, 0f)); // mass = 0: static body

		Gdx.input.setInputProcessor(new InputMultiplexer(this, new GestureDetector(this))); // added by me (to use tap to shoot)
	}
	...
	public void render(boolean update) {
		...
		// set camera to follow player camera = PerspectiveCamera
		Vector3 pTrans = character.transform.getTranslation(new Vector3()); // get the player position
		camera.position.set(pTrans.x + 20f, pTrans.y + 20f, pTrans.z + 0f);
		camera.lookAt(pTrans);
		camera.update();
	...
	public void toggleDebugMode() {
		if (world.getDebugMode() == DebugDrawModes.DBG_NoDebug) {
			setDebugMode(DebugDrawModes.DBG_DrawWireframe | DebugDrawModes.DBG_DrawFeaturesText | DebugDrawModes.DBG_DrawText | DebugDrawModes.DBG_DrawContactPoints | DebugDrawModes.DBG_DrawNormals);
